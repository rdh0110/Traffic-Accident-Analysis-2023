...
# 요일별 빈도수 계산
weekday_counts = df['발생지시도'].value_counts().reindex(['서울','경기', '경남', '경북', '세종', '충남', '충북', '전북', '전남','대구', '인천', '광주', '부산', '제주','울산', '대전','강원' ])
...
plt.xlabel('발생지시도', fontproperties=font_prop) 
plt.ylabel('빈도수', fontproperties=font_prop)  
plt.title('지역에 따른 교통사고 발생 수', fontproperties=font_prop)  # 그래프 제목
